...
- Killing Claude's commit attribution takes **four** keys, not one. `attribution.sessionUrl: false` is a separate gate from `attribution.commit`/`pr`: Claude Code builds the trailer as ``commit ? `${commit}\nClaude-Session: ${url}` : `Claude-Session: ${url}` ``, so an *empty* commit footer makes the session URL the **only** trailer instead of removing it (anthropics/claude-code#77830, closed as working-as-designed). Both installers set all four — `src/startup.sh` for Docker, `src/apply-attribution.ps1` for native Windows. The equivalent escape hatch is `CLAUDE_CODE_SUPPRESS_SESSION_ATTRIBUTION=1`.
- Settings-writing scripts on Windows must merge, never overwrite: `apply-attribution.ps1` and `install-windows-statusline.bat` both touch the same `%USERPROFILE%\.claude\settings.json`. `apply-attribution.ps1` writes a `.bak` first, bails out with exit 1 on invalid JSON, and emits UTF-8 **without** BOM (`ConvertTo-Json` escapes non-ASCII to `\uXXXX`, so nothing is lost).
- `$HOME` is read-only in PowerShell — you cannot override it to test these scripts. Launch a child `powershell.exe` with `USERPROFILE` set and `HOMEDRIVE`/`HOMEPATH`/`HOME` removed from its environment instead. Also note PS 5.1 and PS 7 serialize JSON differently (indent width, `'` escaping), so comparing serialized strings across engines gives false negatives — parse and re-serialize with one engine when diffing.

- Killing Claude's commit attribution takes **four** keys, not one. `attribution.sessionUrl: false` is a separate gate from `attribution.commit`/`pr`: Claude Code builds the trailer as ``commit ? `${commit}\nClaude-Session: ${url}` : `Claude-Session: ${url}` ``, so an *empty* commit footer makes the session URL the **only** trailer instead of removing it (anthropics/claude-code#77830, closed as working-as-designed). Both installers set all four — `src/startup.sh` for Docker, `src/apply-attribution.ps1` for native Windows. The equivalent escape hatch is `CLAUDE_CODE_SUPPRESS_SESSION_ATTRIBUTION=1`.
- Settings-writing scripts on Windows must merge, never overwrite: `apply-attribution.ps1` and `install-windows-statusline.bat` both touch the same `%USERPROFILE%\.claude\settings.json`. `apply-attribution.ps1` writes a `.bak` first, bails out with exit 1 on invalid JSON, and emits UTF-8 **without** BOM (`ConvertTo-Json` escapes non-ASCII to `\uXXXX`, so nothing is lost).
- `$HOME` is read-only in PowerShell — you cannot override it to test these scripts. Launch a child `powershell.exe` with `USERPROFILE` set and `HOMEDRIVE`/`HOMEPATH`/`HOME` removed from its environment instead. Also note PS 5.1 and PS 7 serialize JSON differently (indent width, `'` for `'`), so comparing serialized strings across engines gives false negatives — parse and re-serialize with one engine when diffing.
...
- MCP seeding is **re-entrant**: `startup.sh` (re)seeds when `~/.codex/config.toml` is missing OR lacks any `[mcp_servers.*]` table — because Codex writes its own `config.toml` (model/settings) on first run, and a naive seed-if-missing would then leave the MCP block absent forever. Verified against codex `mcp list` (0.101.x/0.144.x): all five servers load from the seeded/merged file. If MCP tools seem absent, the config almost certainly wasn't seeded — rebuild the image (`codex-docker --rebuild`) and/or check `~/.codex/config.toml` inside the container.
